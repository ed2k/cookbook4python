#!/usr/bin/env python
""" this is updated more towards converting local saved IM logs to emails.
"""
...
def fix_email(line):
    ...
    for email in line.split(';'):
      ...
      email = email.translate(maketrans('[] &()<>=/\\','__._____.__'))
      if email.find('@') == -1: email = email+'@a.jp'
      r.append(email);
    #if len(r)>4: print left+';'.join(r)
    return ';'.join(r)
def fix_header2(msg):
	# assume msg contains only header part
    ...
    for i,line in enumerate(headers):
    	line = line.strip()
        f = line.split()
        if f[0].find(":")==-1: print line
        if len(f) > 1 :
          #print f
          if f[0] in ['To:']: mto = fix_email(' '.join(f))
          elif f[0] in ['Subject:']: msubject = ' '.join(f)
          elif f[0] in ['From:']: mfrom = fix_email(' '.join(f))
          elif f[0].upper() in ['CC:']: r.append(fix_email(' '.join(f)))
          elif f[0] in ['Sent:','Date:'] and len(mdate) == 0:
