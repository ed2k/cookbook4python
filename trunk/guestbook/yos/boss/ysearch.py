...
SEARCH_API_URL = CONFIG["uri"].rstrip("/") + "/%s/v%d/%s?start=%d&count=%d&lang=%s&region=%s&filter=%s&dimensions=%s" + "&appid=" + CONFIG["appid"]
...
def search(command, vertical="web", version=1, start=0, count=10, lang="en", region="us", filter="-porn-hate", dimensions="small", more={}):
  """
...
def suggest(query):
	url = SUGGEST_API_URL % quote_plus(query)
	data = load(url)
	if data:
		return simplejson.loads(data)

def glue(query):
	url = GLUE_API_URL % quote_plus(query)
	data = load(url)
	if data:
